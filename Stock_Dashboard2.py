...
if instrument == 'NSE Equity Market':
    ...
    if data_info == 'equity_list' or data_info == 'fno_equity_list' or data_info == 'market_watch_all_indices' or data_info ==  'nifty50_equity_list' or data_info in ('index_data',
    ...
    if data_info == 'bhav_copy_equities' or data_info == 'bhav_copy_with_delivery':
        date = st.sidebar.text_input('Date','12-12-2023')
...
if instrument == 'NSE Derivatives Market':
    data_info = st.sidebar.selectbox('Data to Extract',options=('expiry_dates_future','expiry_dates_option_index','fii_derivatives_statistics',
                                                                ...
                                                                'participant_wise_open_interest','participant_wise_trading_volumn'))
    if data_info == 'expiry_dates_future' or data_info == 'expiry_dates_option_index':
        data = getattr(derivatives,data_info)()
    if data_info == 'fii_derivatives_statistics' or data_info == 'fno_bhav_copy' or data_info == 'participant_wise_open_interest':
        date = st.sidebar.text_input('Trade Date','16-02-2024')
    ...
    if data_info == 'nse_live_option_chain':
        ticker = st.sidebar.text_input('Ticker','NIFTY')
        expiry_date = st.sidebar.text_input('Expiry Date','15-02-2024')
        data = derivatives.nse_live_option_chain(ticker,expiry_date)
    if data_info == 'participant_wise_trading_volumn':
        trade_date = st.sidebar.text_input('Trade Date','15-02-2024')
        data = derivatives.participant_wise_trading_volume(trade_date)
